...
def local_time():
    """Returns current time as HH:MM:SS in UTC+3 (Sofia/Istanbul timezone)."""
...
def main_dashboard():
    try:
        ...
        # --- ВСИЧКИ 6 МЕТРИКИ НА ЕДИН РЕД ---
        live_price = st.session_state.get('live_price', current_price)
        cached_rsi = st.session_state.get('last_rsi', current_rsi)
        rsi_str = f"{cached_rsi:.2f}" if cached_rsi is not None else "—"
        price_str = f"${live_price:,.2f}" if live_price else f"${current_price:,.2f}"

        col1, col2, col3, col4, col5, col6 = st.columns(6)
        with col1:
            st.metric(label="⚡ Цена ETH", value=price_str)
        with col2:
            st.metric(label=f"📊 RSI ({timeframe})", value=rsi_str)
        with col3:
            st.metric(label="💰 Портфейл USDT", value=f"${st.session_state.paper_usdt:,.2f}")
        with col4:
            st.metric(label="🪙 Наличен ETH", value=f"{st.session_state.paper_eth:.4f}")
        with col5:
        ...
        if st.session_state.paper_eth > 0.001 and st.session_state.avg_buy_price > 0:
            ...
            # Запазваме в сесията — sidebar се рендира извън фрагмента
            ...
            if current_price <= stop_level:
                ...
                msg = f"[{local_time()}] [TRAILING STOP] | Продадени {st.session_state.paper_eth:.4f} ETH на ${current_price:.2f} (P/L: {unrealized_pl:+.2f}%) | RSI: {current_rsi:.2f}"
...
try:
    ...
    if st.sidebar.button("🟩 Ръчно КУПУВА", use_container_width=True):
        ...
        if st.session_state.paper_usdt >= cost:
            ...
            msg = f"[{local_time()}] 👤 МАНУАЛ КУПУВА | {manual_amount:.3f} ETH | ${current_price_manual:.2f} | RSI: {current_rsi_manual:.2f}"
            st.session_state.logs.append(msg)
            logging.info(msg)
            save_portfolio()
            st.rerun()
    ...
    if st.sidebar.button("🟨 Ръчно ПРОДАВА", use_container_width=True):
        if st.session_state.paper_eth >= manual_amount:
            ...
            msg = f"[{local_time()}] 👤 МАНУАЛ ПРОДАЖБА | {manual_amount:.3f} ETH | ${current_price_manual:.2f} | RSI: {current_rsi_manual:.2f}"
            st.session_state.logs.append(msg)
            logging.info(msg)
            if st.session_state.paper_eth < 0.001:
                st.session_state.avg_buy_price = 0
    ...
    if st.sidebar.button("🚨 ПАНИК СЕЛ (Продай всичко)", use_container_width=True):
        if st.session_state.paper_eth > 0:
            ...
            msg = f"[{local_time()}] 🚨 PANIC SELL | {st.session_state.paper_eth:.4f} ETH на ${current_price_manual:.2f} | RSI: {current_rsi_manual:.2f}"
            st.session_state.logs.append(msg)
            logging.info(msg)
            st.session_state.paper_eth = 0
            st.session_state.avg_buy_price = 0
...
# --- SIDEBAR РИСК МОНИТОРИНГ (извън фрагмент) ---
if st.session_state.paper_eth > 0.001 and st.session_state.avg_buy_price > 0:
    st.sidebar.markdown("---")
    st.sidebar.subheader("🛡️ Мониторинг на риска")
    st.sidebar.write(f"Средна цена: **${st.session_state.get('risk_avg_price', st.session_state.avg_buy_price):.2f}**")
