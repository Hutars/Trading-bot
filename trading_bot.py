...
def live_price_widget():
    ...
    with col3:
        st.metric(label="💰 Портфейл USDT", value=f"${st.session_state.get('paper_usdt', 0):,.2f}")
    with col4:
        st.metric(label="🪙 Наличен ETH", value=f"{st.session_state.get('paper_eth', 0):.4f}")
    with col5:
        st.metric(label="📈 Win Rate", value=f"{win_rate:.1f}%", help=f"Общо сделки: {total_trades}")
    with col6:
        st.metric(label="📊 Profit Factor", value=pf_str)
...
def main_dashboard():
    try:
        ...
        # Кешираме RSI — метриките се рендират от live_price_widget на 10 секунди
        st.session_state.last_rsi = current_rsi
